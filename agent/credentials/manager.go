...
type credentialsManager struct {
	...
	// externalInstanceCredentialsId tracks task identifiers associated with external instance credentials, if applicable.
	// Primarily used for the external instance credentials handler to verify the call is associated with a task.
...
func (manager *credentialsManager) SetTaskExternalCredentialsId(id string) {
	manager.externalCredentialsLock.Lock()
	defer manager.externalCredentialsLock.Unlock()

	manager.taskExternalInstanceCredentialIds[id] = true
}

// ValidateExternalCredentialsId checks that the given id exists
